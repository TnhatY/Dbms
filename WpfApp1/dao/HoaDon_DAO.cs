...
namespace Do_an.dao
{
    class HoaDon_DAO
    {
		...
		public DataTable doanhThuNgay(int thang, int nam)
		{
			string query = $"SELECT * FROM dbo.func_tinhDoanhThuNgay({thang}, {nam})";
			return new ConnectDB().getAllData(query);
		}

		public DataTable doanhThuThang(int nam)
		{
			string query = $"SELECT * FROM dbo.func_tinhDoanhThuThang({nam})";
			return new ConnectDB().getAllData(query);
		}

		public DataTable doanhThuNam()
		{
			string query = $"SELECT * FROM dbo.func_tinhDoanhThuNam()";
			return new ConnectDB().getAllData(query);
		}
